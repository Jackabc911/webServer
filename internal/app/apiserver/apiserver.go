...
func (s *APIServer) configureRouter() {
	...
	s.router.HandleFunc(prefix+"/users", s.GetAllUsers).Methods("GET")

	s.router.HandleFunc("/", s.GetHome).Methods("GET")

	s.router.PathPrefix("/static/").Handler(http.StripPrefix("/static/", http.FileServer(http.Dir("./website/static/"))))
